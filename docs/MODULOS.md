...
Card "🛠️ Eliminar actividades pendientes" en la pestaña Calendario de `empresa.html`, visible únicamente cuando `miPerfil.role === 'super_admin'` (herramienta de soporte de la plataforma, no del cliente). Lista las actividades con `status='pendiente'` de la empresa con checkbox por fila; el botón "Eliminar seleccionadas" hace `update` masivo de `deleted_at` (`js/empresa.js`, `window.eliminarPendientesSeleccionadas`) — **soft delete, reversible**, mismo criterio que el resto del sistema (`DATABASE.md` §2, `v16`). No usa el permiso de borrado físico que `v16` reservó exclusivamente a `super_admin` vía SQL Editor — ese sigue sin exponerse en ninguna pantalla.
